//
// Focus on 3D Terrain Programming - demo 5_2
//
// Author: Johan Gardhage <johan.gardhage@gmail.com>
//
// Geomipmapping with view-frustum culling: patches outside the camera's
// frustum are skipped entirely. Press T to toggle texturing, B to toggle the
...
{
	RETRO.title = "Demo 5_2: Culling a Patch or Two";
...
void DEMO_Initialize(void)
{
	...
	if (!terrain.LoadDetailMap("assets/detailmap.tga")) {
		RETRO_RageQuit("[ERROR] Demo5_2::DEMO_Initialize() Unable to load detail map\n");
	}
...
void DEMO_Render(double deltatime)
{
	...
	// Update and render the terrain (cull patches outside the frustum)
