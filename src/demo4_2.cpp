//
// Focus on 3D Terrain Programming - demo 4_2
//
// Author: Johan Gardhage <johan.gardhage@gmail.com>
//
// Using a lightmap. A precomputed grayscale lightmap is loaded and modulated
// over the terrain. Press T to toggle texturing, B to toggle the detail map.
...
{
	RETRO.title = "Demo 4_2: Using a Lightmap";
...
void DEMO_Initialize(void)
{
	...
	if (!terrain.LoadDetailMap("assets/detailmap.tga")) {
		RETRO_RageQuit("[ERROR] Demo4_2::DEMO_Initialize() Unable to load detail map\n");
	}
	...
	// Set up lightmap lighting
	terrain.SetLightingType(LIGHTMAP);
	if (!terrain.LoadLightMap("assets/lightmap.raw", 128)) {
		RETRO_RageQuit("[ERROR] Demo4_2::DEMO_Initialize() Unable to load light map\n");
	}
