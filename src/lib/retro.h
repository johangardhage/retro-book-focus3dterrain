...
// The free-flying camera shared by most demos
RETRO_Camera camera;
...
void RETRO_Initialize(void)
{
	...
	if (RETRO.showcursor) {
		...
		SDL_HideCursor();
		if (RETRO.usecamera && RETRO.mouselook) {
	...
	if (!RETROGL_Initialize(RETRO.window)) {
...
{
	return RETRO.keystate && key >= 0 && key < SDL_SCANCODE_COUNT && RETRO.keystate[key];
}
...
// Default input for terrain demos that use the shared free-flying camera.
void RETRO_Input(double deltatime)
{
	if (!RETRO.usecamera) {
		return;
	}
	...
	// Mouse look (the first delta after grabbing the mouse is discarded)
	if (RETRO.mouselook) {
		float xrel, yrel;
...
void RETRO_Mainloop(void)
{
	while (!RETRO_QuitRequested()) {
		...
		// Input handling
		RETRO_Input(deltatime);
		if (DEMO_Input) {
			DEMO_Input(deltatime);
		}
